...
<div class="words herotext">
    ...
    On average, that's <?= round(array_sum($days) / (($end - $start) / (60*60*24))) ?> articles per day, <?= round(array_sum($days) / (($end - $start) / (60*60*24*30))) ?> articles per month, or <?= round(array_sum($days) / (($end - $start) / (60*60*24*365))) ?> articles per year. Keep it up! <?php if (Config::EMOJI_ICONS) { echo "💯"; } ?>
</div>
