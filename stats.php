...
<div class="words herotext">
    ...
    You've spent about <strong><?= Helper::makeTimeHumanReadable(Read::getTotalTimeSpent($start, $end)) ?></strong> reading <strong><?= Read::getTotalArticleCount("archived", $start, $end) ?> articles</strong> <?= $periodText ?>.<br>
    On average, that's <?= Helper::makeTimeHumanReadable(Read::getTotalTimeSpent($start, $end) / ((min($time, $end) - max(Read::getFirstArticleTime(), $start)) / (60*60*24))) ?> or <?= round(array_sum($days) / ((min($time, $end) - max(Read::getFirstArticleTime(), $start)) / (60*60*24))) ?> articles per day, <?= round(array_sum($days) / ((min($time, $end) - max(Read::getFirstArticleTime(), $start)) / (60*60*24*30))) ?> articles per month, or <?= round(array_sum($days) / ((min($time, $end) - max(Read::getFirstArticleTime(), $start)) / (60*60*24*365))) ?> articles per year. Keep it up! <?php if (Config::ICON_FONT == "emoji") { echo "💯"; } ?>
</div>
