	...
	<main>
		<?php if ($state === "stats") { ?>
			<div class="stats">
				...
				<div id="articlesperday"></div>
				<?php
					$articlesPerDay = Read::getArticlesPerTime("days", "archived");
					$doysUntilFirstArticle = date("z", Read::getFirstArticleTime());
					$x = range($doysUntilFirstArticle, count($articlesPerDay) + $doysUntilFirstArticle);
					$y = $articlesPerDay;
					// TODO text: date!
				?>
				<script>
					...
					Plotly.newPlot('articlesperday', data, layout, {displayModeBar: false});
				</script>
				...
				<?php
					...
					$y = $articlesPerMonth;
					$text = array_map(function($n) {return date('F', mktime(0, 0, 0, $n, 10));;}, $x); // TODO add year
				?>
				<script>
					var trace1 = {
						x: [<?php echo implode(",", $x); ?>],
